...
async function processMercadoPagoWebhook(req) {
  ...
  if (payment.status === 'approved') {
    ...
    if (!existingOrder.paid_at) { // Essa checagem simples garante que o email não seja reenviado se o MP repetir o webhook
      console.log('[OrderApproved] Iniciando envio de notificação para a administração da loja', { orderId: updatedOrder.id });
      try {
        await sendAdminNotification(orderWithItems);
      } catch (error) {
        console.error('[AdminEmail] Erro não impeditivo ao enviar email de notificação admin:', error.message);
      }
    }
